...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--all', action='store_true', help='Barcha tuzatish va tozalash amallarini tavsiya etilgan tartibda bajaradi.')
        parser.add_argument('--cancel-tasks', action='store_true', help="[1] Barcha faol Celery va Celery Beat tasklarini bekor qiladi va tozalaydi.")
        parser.add_argument('--cleanup-data', action='store_true', help="[2] Keraksiz hujjatlarni o'chiradi (50MB dan katta) va o'chirilmagan fayllarni qayta o'chirishga urinadi.")
        parser.add_argument('--fix-urls', action='store_true', help="[3] Hujjatlarning 'parse_file_url' kengaytmasini 'poster_url' bilan moslashtiradi.")
        parser.add_argument('--fix-states', action='store_true', help="[4] Barcha hujjatlar holatini qat'iy qoidalar asosida to'g'rilaydi.")
        parser.add_argument('--dry-run', action='store_true', help="O'zgarishlarni bajarmasdan, nima qilinishini ko'rsatadi.")
        parser.add_argument('--force', action='store_true', help="Celery tasklarini majburiy to'xtatish (terminate) uchun.")
    ...
    def run_cleanup_data(self, dry_run):
        ...
        for doc in Document.objects.all():
            ...
            if file_size_str and self._parse_file_size_to_bytes(file_size_str) > 50 * 1024 * 1024:
                large_docs_count += 1
                if not dry_run: doc.delete()
        if large_docs_count > 0: self.stdout.write(self.style.SUCCESS(f"   ✅ Hajmi 50MB dan katta bo'lgan {large_docs_count} ta hujjat o'chirildi."))
        retry_delete_count = 0
        done_statuses = ['completed', 'skipped']
        docs_to_retry_delete = Document.objects.filter(download_status__in=done_statuses, parse_status__in=done_statuses, index_status__in=done_statuses, telegram_status__in=done_statuses).exclude(delete_status='completed')
        for doc in docs_to_retry_delete:
            ...
            if os.path.exists(file_path):
                ...
                except Exception as e: logger.error(f"Faylni o'chirishda xato: {file_path} - {e}")
        if retry_delete_count > 0: self.stdout.write(self.style.SUCCESS(f"   ✅ {retry_delete_count} ta fayl qaytadan muvaffaqiyatli o'chirildi."))
    ...
    def run_fix_urls(self, dry_run):
        ...
        if fixed_count > 0: self.stdout.write(self.style.SUCCESS(f"   ✅ {fixed_count} ta hujjatning URL manzili tuzatildi."))
    ...
    def run_fix_states(self, dry_run):
        ...
        if count_stuck > 0:
            ...
            self.stdout.write(self.style.SUCCESS(f"   ✅ 1 soatdan ortiq qotib qolgan {count_stuck} ta 'pipeline' to'xtatildi."))
        fixed_to_true = 0
        fixed_to_false = 0
        try:
            with transaction.atomic():
                ...
                for doc in Document.objects.select_for_update().all():
                    is_perfectly_completed = all([doc.download_status == 'completed', doc.parse_status == 'completed', doc.index_status == 'completed', doc.telegram_status == 'completed', doc.telegram_file_id is not None])
        ...
        if fixed_to_true > 0: self.stdout.write(self.style.SUCCESS(f"   ✅ {fixed_to_true} ta hujjat 'completed=True' holatiga to'g'rilandi."))
        if fixed_to_false > 0: self.stdout.write(self.style.SUCCESS(f"   ✅ {fixed_to_false} ta hujjat 'completed=False' holatiga to'g'rilandi."))
    ...
    def _parse_file_size_to_bytes(self, file_size_str):
        ...
        elif unit == 'MB': return size * 1024 * 1024
        elif unit == 'KB': return size * 1024
        return 0
    ...
    def _fix_url_extension(self, doc, poster_url, dry_run):
        try:
            ...
            if poster_ext_nodot in ALL_EXTS_LOWER and file_ext_nodot == poster_ext_nodot and file_ext != poster_ext:
                ...
                return True
        except Exception as e: logger.warning(f"URL tuzatishda xato: {doc.id} - {e}")
        return False